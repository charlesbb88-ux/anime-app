...
export type GlobalLogModalPhoneProps = {
    ...
    // NEW: watched/logged date (YYYY-MM-DD)
    loggedOnDate: string;
            ...
            <div className="sticky top-0 z-20 border-b border-white/10 bg-zinc-950/95 backdrop-blur">
                <div className="flex items-center justify-between px-4 py-3">
                    <button
                        type="button"
                        onClick={onClose}
                        className="rounded-md px-2 py-1 text-sm text-zinc-200 hover:bg-white/5"
                    >
                        Close
                    </button>

                    <div className="min-w-0 px-3 text-center">
                        <div className="text-[11px] font-medium text-zinc-400">Log / Review</div>
                        <div className="truncate text-sm font-semibold text-white">{title ?? "Log"}</div>
                    </div>
                ...
                <div className="px-4 pb-3">
                    <div className="flex items-center gap-3">
                        <div className="h-[72px] w-[52px] flex-none overflow-hidden rounded-[2px] bg-zinc-900">
                            {posterUrl ? (
                                <img src={posterUrl} alt={title ?? "Poster"} className="h-full w-full object-cover" />
                            ) : null}
                        </div>
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            {showJournalCheckbox ? (
                                <div className="flex flex-col gap-2">
                                    ...
                                    {/* NEW: date picker (only when checkbox ON) */}
                                    {logWatchToActivity ? (
                                        <div className="pl-6">
                            ...
                            {showLikeAndStars ? (
                                <div className="mt-2 flex items-center justify-between">
                                    ...
                                    >
                                        <Heart className={["h-6 w-6", heartColor, heartFill].join(" ")} />
                                        ...
                                        {Array.from({ length: 5 }).map((_, i) => {
                                            ...
                                            return (
                                                <div key={starIndex} className="relative">
                                                    <StarVisual filledPercent={filled} dim={saving} size={24} />
            ...
            <form onSubmit={handleSubmit} className="h-[calc(100dvh-160px)] overflow-y-auto px-4 py-4">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        disabled={saving}
                        className={[
                            "min-h-[240px] w-full resize-none rounded-md border border-zinc-800 bg-zinc-950/40 px-3 py-3 text-sm text-white outline-none focus:border-zinc-500",
                            saving ? "opacity-60" : "",
                        ].join(" ")}
                    />
                </div>
                ...
                {/* bottom padding so content doesn't hide behind iOS UI */}
                <div className="h-10" />
            </form>
