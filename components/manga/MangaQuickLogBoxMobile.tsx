// components/manga/MangaQuickLogBoxMobile.tsx
"use client";
...
export default function MangaQuickLogBoxMobile({
  mangaId,
  totalChapters,
  onOpenLog,
  widthClassName,
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.error("MangaQuickLogBoxMobile: error loading chapters", error);
        setChapters([]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (!error && data) {
        ...
        if (error) console.warn("MangaQuickLogBoxMobile: volume map load failed", error);
        setVolumeMap(null);
      }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("MangaQuickLogBoxMobile: failed to load chapter log counts", error);
        setLogCounts({});
        return;
      }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("MangaQuickLogBoxMobile: failed to load chapter review counts", error);
        setReviewCounts({});
        return;
      }
  ...
  async function quickLogChapter(ch: ChapterRow) {
    ...
    try {
...

      {
        ...
        if (del.error) {
          console.error("[MangaQuickLogBoxMobile] watched mark delete failed:", del.error);
        ...
        if (ins.error) {
          console.error("[MangaQuickLogBoxMobile] watched mark insert failed:", ins.error);
          setMsg("Couldn’t log (watched mark failed).");
          return;
        }
      }

      {
        ...
        if (error) {
          console.error("[MangaQuickLogBoxMobile] createMangaChapterLog failed:", error);
