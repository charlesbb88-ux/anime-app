...
export default function MangaQuickLogRowMobile({
  mangaId,
  slug,
  chapters,
  canInteract,
  ...
  const [autoPhase, setAutoPhase] = useState<"idle" | "swiping" | "holding" | "returning">(
    "idle"
  );
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("[MangaQuickLogRowMobile] failed to load chapter logs:", error);
        setMaxLoggedNumber(null);
        return;
      }
  ...
  async function createWatchedMark(params: { userId: string; chapterId: string }) {
    ...
    if (del.error) {
      console.error("[MangaQuickLogRowMobile] watched mark delete failed:", del.error);
    ...
    if (ins.error) {
      console.error("[MangaQuickLogRowMobile] watched mark insert failed:", ins.error);
  ...
  async function quickLogWithWatched(ch: ChapterRow) {
    ...
    try {
      ...
      if (error) {
        console.error("[MangaQuickLogRowMobile] createMangaChapterLog failed:", error);
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    const next = clamp(startSwipeXRef.current + dx, 0, swipeMaxPx);
    setSwipeX(next);
  }
  ...
  async function onPointerUpOrCancel(e: React.PointerEvent) {
    ...
    if (swipeX >= swipeCommitPx && !committedRef.current) {
  ...
  const transition = useMemo(() => {
    ...
    if (autoPhase === "swiping")
      return `transform ${AUTO_SWIPE_MS}ms cubic-bezier(0.2, 0.9, 0.2, 1)`;
    if (autoPhase === "returning")
      return `transform ${AUTO_RETURN_MS}ms cubic-bezier(0.2, 0.9, 0.2, 1)`;
