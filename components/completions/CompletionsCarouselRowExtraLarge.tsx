// components/completions/CompletionsCarouselRowExtraLarge.tsx
...
export default function CompletionsCarouselRowExtraLarge({ items }: Props) {
    ...
    // =========
    // EXTRA LARGE CONSTANTS
    // =========
    const CARD_W = 231; // 210 * 1.1
    const CARD_H = 332; // 302 * 1.1
    const STAGE_H = 380; // 345 * 1.1

    // stack feel (keep same)
    ...
    // target number of cards in the spread (auto-reduces if it can’t fit)
    const WINDOW_TARGET = 6;

    // same inset logic as the others
    const DECK_INSET = 18;

    // ✅ enforce real air between spread cards so they never glue together
    const SPREAD_GAP = 18;

    // overview layout (non-hover)
    const OVERVIEW_PAD = 18;
    const OVERVIEW_MIN_STEP = 10;
    const OVERVIEW_MAX_STEP = 30;
    ...
    const overviewBand = useMemo(() => {
        ...
        return { startX, step };
    }, [stageW, items.length, CARD_W]);

    // deck mode indices (REVERSED WINDOW — same as small/large)
        ...
        <section className="w-full">
            <div className="mt-1">
                <div
                    ...
                    tabIndex={0}
                    role="group"
                    aria-label="Completions carousel (extra large)"
                >
