    ...
    <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-2 text-sm text-white/65">
            ...
            {mode === "sent" ? (
              <>
                Waiting for response • expires {formatRelativeDate(challenge.expires_at)}
              </>
            ) : null}

            {mode === "ready" ? (
              <>
                Battle created {formatRelativeDate(challenge.responded_at ?? challenge.updated_at)}
              </>
            ) : null}
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {mode === "ready" && challenge.battle_id ? (
          <button
            type="button"
            onClick={() => onWatch(challenge.battle_id!)}
            disabled={watching}
            className="rounded-xl border border-white/10 bg-white px-4 py-2 text-sm font-semibold text-black transition hover:bg-white/90 disabled:cursor-not-allowed disabled:opacity-50"
          >
...
export default function ChallengeInbox({ className = "" }: Props) {
  ...
  function handleWatch(battleId: string) {
    const key = `ready:${battleId}:watch`;
    setLoadingAction(key);
  ...
  return (
    <div className={className || "rounded-2xl border border-white/10 bg-black px-4 py-4"}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-[0.2em] text-white/45">
            Battles
          </div>
          <h2 className="mt-1 text-xl font-semibold text-white">Battle inbox</h2>
        </div>
