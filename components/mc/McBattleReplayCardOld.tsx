...
export default function McBattleReplayCard({
  ...
  const initialChallengerHp = safeNumber(
    challengerSnapshot?.combat_stats?.hp,
    0
  );
  ...
  const timeline = useMemo(() => {
    ...
    return hitEvents.map((event, index) => {
      ...
      return {
        step: index + 1,
        actor_side: event.attacker === "left" ? "challenger" : "defender",
  ...
  const challengerAvatarLayers: CharacterAvatarLayer[] = [];
  const defenderAvatarLayers: CharacterAvatarLayer[] = [];
  ...
  const challengerHpPercent = useMemo(() => {
    ...
    return clampPercent(
      (challengerState.currentHp / challengerState.maxHp) * 100
    );
    ...
    <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-white">
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.06),rgba(255,255,255,0.015)_35%,rgba(0,0,0,0.4)_70%)]">
        ...
        <div className="relative h-[460px] w-full">
          <div
            className={`absolute bottom-[14px] left-[20%] transition-all duration-500 ${
              challengerIsActing
                ? "translate-x-[110px] scale-[1.06]"
          ...
          <div className="pointer-events-none absolute bottom-[24px] left-1/2 z-10 -translate-x-1/2 rounded-full border border-white/10 bg-black/40 px-5 py-3 text-sm font-semibold text-white/75">
            VS
          </div>

          <div
            className={`absolute bottom-[14px] right-[20%] transition-all duration-500 ${
              defenderIsActing
                ? "-translate-x-[110px] scale-[1.06]"
                : "translate-x-0 scale-100"
        ...
        {activeEvent ? (
          <div>
            <span className="font-medium text-white">
              {activeEvent.actor_side}
            </span>
            {" hit "}
            <span className="font-medium text-white">
              {activeEvent.target_side}
            </span>
            {" for "}
