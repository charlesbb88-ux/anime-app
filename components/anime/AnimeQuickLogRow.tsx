// components/anime/AnimeQuickLogRow.tsx
"use client";
...
export default function AnimeQuickLogRow({
  animeId,
  slug,
  episodes,
  canInteract,
  ...
  // ✅ when user uses Review modal, remember the episode and poll for its log
  ...
  const completedEpisodes = useMemo(() => {
    ...
    // count how many episodes have episode_number <= maxLoggedNumber
    // (this avoids assuming there are no gaps)
    let count = 0;
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("[AnimeQuickLogRow] failed to load episode logs:", error);
        setMaxLoggedNumber(null);
        return;
      }
  ...
  // ✅ poll for the log row after Review modal is used
  async function startReviewPoll(ep: EpisodeRow) {
    ...
    reviewPollTimerRef.current = setInterval(async () => {
      ...
      // ✅ check if the episode log exists yet
      const { data, error } = await supabase
        .from("anime_episode_logs")
        .select("id")
        .eq("anime_id", animeId)
      ...
      // ✅ log detected -> navigate to next episode page
      stopReviewPoll();
      pendingReviewEpisodeRef.current = null;

      // update local maxLogged too (so nextEpisode updates instantly)
  ...
  async function createWatchedMark(params: { userId: string; episodeId: string }) {
    ...
    if (del.error) {
      console.error("[AnimeQuickLogRow] watched mark delete failed:", del.error);
    ...
    if (ins.error) {
      console.error("[AnimeQuickLogRow] watched mark insert failed:", ins.error);
  ...
  async function quickLogWithWatched(ep: EpisodeRow) {
    ...
    try {
      ...
      if (error) {
        console.error("[AnimeQuickLogRow] createAnimeEpisodeLog failed:", error);
