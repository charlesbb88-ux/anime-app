// components/anime/AnimeQuickLogBox.tsx
"use client";
...
import AnimeQuickLogRow from "@/components/anime/AnimeQuickLogRow";
...
export default function AnimeQuickLogBox({
  animeId,
  totalEpisodes,
  onOpenLog,
  widthClassName,
  ...
  // 1) Load episodes (normalize episode_number to number)
  useEffect(() => {
    if (!animeId) return;
    ...
    async function run() {
      ...
      if (error || !data) {
        console.error("AnimeQuickLogBox: error loading episodes", error);
        setEpisodes([]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // NOTE: rename this table/source if your project uses different names
      const { data, error } = await supabase
      ...
      if (!error && data) {
        ...
        if (error) console.warn("AnimeQuickLogBox: season map load failed", error);
        setSeasonMap(null);
      }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("AnimeQuickLogBox: failed to load episode log counts", error);
        setLogCounts({});
        return;
      }
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (error || !data) {
        console.warn("AnimeQuickLogBox: failed to load episode review counts", error);
        setReviewCounts({});
        return;
      }
  ...
  // Reuse the same grouping helper:
  // - If you have season mapping, it behaves like "volume mapping"
  // - Otherwise, it falls back to chunk groups
  const navGroups = useMemo<NavGroup[]>(() => {
    if (!seasonMapLoaded) return [];
    return buildChapterNavGroups({
      volumeMap: seasonMap, // season map uses same shape
      totalChapters: cappedTotal, // total episodes
      chunkSize: 25,
    });
  ...
  // ✅ bucket by integer part so decimals show inside the right group
  ...
  async function quickLogEpisode(ep: EpisodeRow) {
    ...
    try {
      ...
      // watched mark (episode scoped)
      {
        ...
        if (del.error) {
          console.error("[AnimeQuickLogBox] watched mark delete failed:", del.error);
        ...
        if (ins.error) {
          console.error("[AnimeQuickLogBox] watched mark insert failed:", ins.error);
          setMsg("Couldn’t log (watched mark failed).");
          return;
        }
      }
      ...
      // create episode log
      {
        ...
        if (error) {
          console.error("[AnimeQuickLogBox] createAnimeEpisodeLog failed:", error);
                          ...
                          <div className="min-h-0 overflow-hidden">
                            <div className="px-0 pb-0">
                              <div
                                ...
                                className={[
                                  "aqb-scroll max-h-[320px] overflow-y-auto",
