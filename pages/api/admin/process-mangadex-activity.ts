...
function getBaseUrl(req: NextApiRequest) {
  // Prefer SITE_URL (what you said you have in Vercel), then NEXT_PUBLIC_SITE_URL, then request origin.
  const envBase =
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 1) Pull newest activity (recent chapters table)
    ...
    // 2) Refresh each manga specifically (THIS is the fix)
    for (const mdId of uniqueIds) {
      const url =
        ...
        `&force=1` +
        `&mode=chapter` +
        `&state_id=activity_${mdId}` + // isolate cursor per manga id
        `&max_pages=1` +
        `&hard_cap=1`;
