...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const idToTime = new Map(
            (rows || [])
                .filter(r => r.mangadex_manga_id && r.updated_at)
                .map(r => [r.mangadex_manga_id, r.updated_at])
        );
        ...
        for (const mdId of uniqueIds) {
            ...
            // basic classification (can get smarter later)
            const action =
                text.includes('"inserted"') ? "inserted" :
                    text.includes('"updated"') ? "updated" :
                        "processed";

            await supabaseAdmin.from("mangadex_sync_log").insert({
                mangadex_manga_id: mdId,
                manga_id: null, // optional: can be populated later
                action,
                md_updated_at: idToTime.get(mdId) ?? new Date().toISOString(),
                details: {
                    status: r.status,
                    response_sample: text.slice(0, 500),
                },
            });
        }
