...
export default function McDotFightTestPage() {
  ...
  const leftLoadout = useMemo(() => {
    return normalizePaperdollLoadout(battleRow?.challenger_snapshot?.paperdoll);
  }, [battleRow]);

  const rightLoadout = useMemo(() => {
    return normalizePaperdollLoadout(battleRow?.defender_snapshot?.paperdoll);
  }, [battleRow]);
  ...
  const buildReplay = async () => {
    ...
    const typed = data as BattleRow;

    setBattleRow(typed);
    setReplay(typed.replay_data?.dot_replay ?? null);

    const nextReplay = typed.replay_data?.dot_replay;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {battleRow && (
          <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-white/80">
            ...
            <div className="mt-3 text-sm font-medium text-white">Challenger Paperdoll</div>
            <pre className="mt-1 overflow-auto rounded-lg border border-white/10 bg-black/20 p-3 text-[11px]">
              {JSON.stringify(battleRow.challenger_snapshot?.paperdoll ?? null, null, 2)}
            </pre>

            <div className="mt-3 text-sm font-medium text-white">Defender Paperdoll</div>
            <pre className="mt-1 overflow-auto rounded-lg border border-white/10 bg-black/20 p-3 text-[11px]">
              {JSON.stringify(battleRow.defender_snapshot?.paperdoll ?? null, null, 2)}
            </pre>
            ...
            <div>Frame Count: {battleRow.replay_data?.dot_replay?.frames?.length ?? 0}</div>
            <div>Hit Event Count: {battleRow.replay_data?.dot_replay?.hitEvents?.length ?? 0}</div>
            <div>Event Count: {battleRow.replay_data?.dot_replay?.events?.length ?? 0}</div>
          </div>
        )}
