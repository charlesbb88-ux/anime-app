...
export type MangaDexManga = {
  ...
  attributes: {
    ...
    // ✅ present on list responses (and useful for cursor mode)
    createdAt?: string;
    updatedAt?: string;
...
// MangaDex validation for updatedAtSince wants EXACTLY:
// YYYY-MM-DDTHH:MM:SS  (no milliseconds, no timezone suffix)
// Example: 1970-01-01T00:00:00
function toMangaDexDateTime(input: string) {
  const d = new Date(input);
  if (!Number.isFinite(d.getTime())) {
    // If caller already passed a MangaDex-format string, keep it.
    return String(input);
  }
...
export function splitTags(m: MangaDexManga): { genres: string[]; themes: string[] } {
  ...
  for (const t of tags) {
    ...
    if (group === "genre") genres.push(name);
      ...
      // if group missing/unknown, ignore to stay conservative / clean.
    }
  }
